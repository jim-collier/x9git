...
function fMain(){
	...
	local -r goVersion="$(go version | grep -iPo "[0-9\.]+" 2>/dev/null | head -n 1 || true)"
...
#	CGO_LDFLAGS="$(fStrNormalize_byecho "${CGO_LDFLAGS}")"  #.........: Normalize string
	...
	ldFlags="${ldFlags} -X main.GoVersion=${goVersion}"  #................................: Inject value
...
#	goTags="$(fStrNormalize_byecho "${goTags}")"  #...: Normalize string
	...
	if [[ -n "${CGO_CFLAGS}"  ]]; then fEcho; fEcho_Clean    "CGO_CFLAGS:\n${CGO_CFLAGS}"    ; fi
	if [[ -n "${CGO_LDFLAGS}" ]]; then fEcho; fEcho_Clean    "CGO_LDFLAGS:\n${CGO_LDFLAGS}"  ; fi
	if [[ -n "${ldFlags}"     ]]; then fEcho; fEcho_Clean    "ldFlags:\n${ldFlags}"          ; fi
	if [[ -n "${goTags}"      ]]; then fEcho; fEcho_Clean    "goTags:\n${goTags}"            ; fi
