...
- ✅ The SSH identity line named the local login instead of the account being acted as.
	- It asked `ssh -G` about the bare host. With no user in the target, ssh answers with the OS login name, so a push as the second account displayed as `<os-login>@github.com`. Reported from the field.
